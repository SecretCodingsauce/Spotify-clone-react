            ...
            <nav id="nav" className="flex justify-between items-center bg-black text-white p-2 fixed w-full top-0 left-0 ">
                <NavLink to="/">
                    <div className="flex items-center">
                        <img className="icon" alt="" src={spotifylogo}></img>
                ...
                {token ? <div className="flex  w-2/5">
                    ...
                    <NavLink to="/SearchPage" className="border-white border-2 rounded rounded-l-none" type="submit" onClick={() => updateSearch(searchKeylol)}><img className="w-5 m-2 " src={searchicon} alt="search"></img></NavLink>
                </div>
                    : <></>}

                {!token ? <div className="flex items-center">
                    <a className="flex items-center" href={LOGIN}>
                        <img alt="" className="icon" src={userpicture}></img>
                        <span className="button ml-3 font-bold">Login</span>
                    </a>
                </div>

                    : <NavLink to="userPage" className="flex items-center">

                        <img className="icon" alt="" src={userData?.images?.length > 0 ? userData.images[0].url : userpicture}></img>
                        <span className="ml-3 font-bold">{userData.display_name}</span>
                    </NavLink>
